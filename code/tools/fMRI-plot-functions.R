...
diff_ctl <- function(x){
  output <- x %>%
    dplyr::select(subject, trauma, CSPM_1, CSPM_2, CSPM_3, CSPM_4) %>%
...
diff_plot <- function(x){
  ggplot(data = x[[1]], aes(x=block, y=mean)) +
    ...
    ylab("BOLD response") +
    ggtitle(paste0(x[[2]],": CS+ minus CS-")) +
...
###############################################################################
# DIFFERENTIAL GRAPH FUNCTIONS FOR FEAR COND - SAFETY SIGNALING; INVERSE
###############################################################################
# additional functions needed to plot the inverse, upwards trajectory

# get just the MP differentials and separate by group... reshaping data 

diff_ctl_safety <- function(x){
  output <- x %>%
    dplyr::select(subject, trauma, CSMP_1, CSMP_2, CSMP_3, CSMP_4) %>%
    gather(key = "block", value = "response", 3:6 ) %>%
    filter(trauma == 0)
  return(output)
}
...
diff_plot_safety <- function(x){
  ggplot(data = x[[1]], aes(x=block, y=mean)) +  ####### group =1 makes geomline work
...
addstimsign <- function(x){
  output <- x %>%
    mutate(stimsign = 
             if_else(substr(stim, 1, 3) == "CSP", "CS+", "CS-"))
  return(output)
}
